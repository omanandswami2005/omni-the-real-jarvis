...
async def main():
            ...
            "local_tools": [
                {
                    ...
                    "parameters": {
                        "region": {"type": "string", "description": "full or region name"}
                    },
                },
                {
                    "name": "open_file",
    ...
    if auth_resp and auth_resp.get("status") == "ok":
        ...
        print(
            f"   ✓ Available tools ({len(tools)}): {tools[:10]}{'...' if len(tools) > 10 else ''}"
        )
    ...
    if chat_auth_resp and chat_auth_resp.get("status") == "ok":
        ...
        await chat_ws.send(
            json.dumps(
                {"type": "text", "content": "Hello, what tools do you have? List them briefly."}
            )
        )
        ...
        action_msgs = [
            ...
            if m.get("type") in ("tool_call", "tool_response", "agent_transfer")
        ]
        if action_msgs:
